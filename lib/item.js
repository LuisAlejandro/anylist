...
class Item {
   ...
   */
	...
	/** Build PBItemQuantity for ListItem; ListItem has quantityPb, not quantity. */
	_quantityPb() {
		...
		if (m) {
			return new this._protobuf.PBItemQuantity({
				amount: m[1].replace(',', '.'),
				unit: m[2],
				rawQuantity: raw,
			});
		}
		return new this._protobuf.PBItemQuantity({ amount: raw, rawQuantity: raw });
	}
	...
	_encode() {
		...
		const qPb = this._quantityPb();
		if (qPb) listItem.quantityPb = qPb;
		...
		if (this._categoryAssignments.length > 0) {
			listItem.categoryAssignments = this._categoryAssignments.map(a => new this._protobuf.PBListItemCategoryAssignment(a));
		}
	 ...
	 * Category assignments are not copied: the new list has only the default "Other" category
	 * until list category structure is copied (not yet supported), so old categoryGroupId/categoryId
	 * would be invalid and show as "None".
	 ...
	 * @param {Map<string, { categoryGroupId: string, categoryId: string }>} [categoryMap] map of "oldGroupId:oldCategoryId" -> new { categoryGroupId, categoryId } for preserving categories on duplicate
	 */
	encodeForCopy(newIdentifier, newListId, categoryMap) {
		const listItem = {
		...
		if (categoryMap && this._categoryAssignments.length > 0) {
			const mapped = this._categoryAssignments
				.map(a => {
					const key = `${a.categoryGroupId}:${a.categoryId}`;
					const next = categoryMap.get(key);
					return next ? { identifier: uuid(), categoryGroupId: next.categoryGroupId, categoryId: next.categoryId } : null;
				})
				.filter(Boolean);
			if (mapped.length > 0) {
				listItem.categoryAssignments = mapped.map(a => new this._protobuf.PBListItemCategoryAssignment(a));
			}
		}
		const qPb = this._quantityPb();
		if (qPb) listItem.quantityPb = qPb;
   ...
   */
	async save(isFavorite = false) {
		const allOps = [];
		const hasQuantityUpdate = this._fieldsToUpdate.includes('quantity');

		if (hasQuantityUpdate) {
			const removeHandler = isFavorite ? 'remove-item' : 'remove-shopping-list-item';
			const removeOp = new this._protobuf.PBListOperation();
			removeOp.setMetadata({ operationId: uuid(), handlerId: removeHandler, userId: this._uid });
			removeOp.setListId(this._listId);
			removeOp.setListItemId(this._identifier);
			removeOp.setListItem(this._encode());
			allOps.push(removeOp);

			const newId = uuid();
			const addHandler = isFavorite ? 'add-item' : 'add-shopping-list-item';
			const addOp = new this._protobuf.PBListOperation();
			addOp.setMetadata({ operationId: uuid(), handlerId: addHandler, userId: this._uid });
			addOp.setListId(this._listId);
			addOp.setListItemId(newId);
			const oldId = this._identifier;
			this._identifier = newId;
			addOp.setListItem(this._encode());
			this._identifier = oldId;
			allOps.push(addOp);
		}

		const simpleFields = this._fieldsToUpdate.filter(f => OP_MAPPING[f]);
		for (const field of simpleFields) {
			if (hasQuantityUpdate) continue;
